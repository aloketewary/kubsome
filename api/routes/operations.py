...
def get_node_ops():
    ...
    kctx = str(ctx.current_context or "")

    # Bolt: O(N) -> O(1) latency relative to nodes by parallelizing and batching pod counts
    with ThreadPoolExecutor(max_workers=2) as executor:
        f_nodes = executor.submit(get_raw_resources, "nodes", kctx)
        f_pods = executor.submit(get_raw_resources, "pods", kctx, namespace=None) # All namespaces
    ...
    # Bolt: Aggregate pod counts by node in memory to avoid N+1 kubectl calls
    pod_counts = {}
