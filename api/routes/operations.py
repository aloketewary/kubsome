...
def get_node_ops():
    ...
    # Bolt: Parallelize fetching nodes and pods to reduce latency from O(N_nodes) to O(1)
    ...
    # Bolt: Aggregate pod counts per node in-memory (O(N_pods)) instead of sequential subprocess calls (O(N_nodes))
    pod_counts = Counter()
    for pod in pod_data.get("items", []):
        node_name = pod.get("spec", {}).get("nodeName")
        if node_name:
            pod_counts[node_name] += 1
