...
export class ScheduleComponent implements OnInit {
  ...
  get activeCount() { return this.schedules.filter(s => s.enabled !== false).length; }
  get pausedCount() { return this.schedules.filter(s => s.enabled === false).length; }

  ngOnInit() { this.load(); }

  load() {
    this.http.get<any>('/api/schedules').subscribe({ next: (res) => { this.schedules = res.schedules || []; } });
  }

  addSchedule() {
    const commands = this.newCommands.split(',').map(c => c.trim()).filter(Boolean);
    this.http.post<any>('/api/schedules', { name: this.newName.trim(), cron: this.newCron.trim(), commands, notify: true }).subscribe({
      next: () => { this.newName = ''; this.newCron = ''; this.newCommands = ''; this.showForm = false; this.load(); },
    });
  }

  removeSchedule(name: string) {
    this.http.delete<any>(`/api/schedules/${name}`).subscribe({ next: () => this.load() });
  }
}
