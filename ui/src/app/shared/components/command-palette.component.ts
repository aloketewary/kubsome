...
export class CommandPaletteComponent implements OnInit {
  ...
  private pages: PaletteItem[] = [
    ...
    { type: 'page', icon: 'pi pi-chart-bar', label: 'Metrics', action: () => this.go('/metrics') },
    ...
    { type: 'page', icon: 'pi pi-sparkles', label: 'AI Assistant', action: () => this.go('/ai') },
  ...
  ngOnInit() {
    ...
    this.api.getPods().subscribe(res => {
      const podItems: PaletteItem[] = res.pods.map(p => ({
        type: 'pod' as const,
        icon: p.status === 'Running' ? 'pi pi-check-circle' : 'pi pi-exclamation-triangle',
        label: p.name,
        hint: p.status,
        action: () => this.go('/pods'),
      }));
  ...
  toggle() {
    ...
    if (this.visible) {
      ...
      this.filtered = this.allItems.slice(0, 8);
      setTimeout(() => {
        document.querySelector<HTMLInputElement>('.palette-input-wrap input')?.focus();
      }, 50);
    }
  }
  ...
  filter() {
    ...
    if (!q) {
      ...
      this.filtered = this.allItems
        .filter(item => item.label.toLowerCase().includes(q) || item.type.includes(q))
        .slice(0, 10);
    }
    this.activeIndex = 0;
  }
